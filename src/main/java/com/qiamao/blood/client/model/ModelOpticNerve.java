...
public class ModelOpticNerve extends ModelBase {
    ...
    public ModelOpticNerve() {
        ...
        // 头部上的两个管子（与头部同体，居中对称）
        this.Head.setTextureOffset(0, 2).addBox(2.1F, -25.0F, -1.5F, 8, 1, 1, 0.0F);
        this.Head.setTextureOffset(0, 0).addBox(-9.9F, -25.0F, -1.5F, 8, 1, 1, 0.0F);

        // 不再预设头部的 X/Y 旋转角度，让它保持原位
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        // 警笛式头部转动
        if (entityIn instanceof EntityOpticNerve) {
            EntityOpticNerve opticNerve = (EntityOpticNerve) entityIn;
            this.Head.rotateAngleY = opticNerve.getHeadSpinAngle();
        }
    }
}
